...
for region in $RegionList
    ...
    rdsRI=$(aws rds describe-reserved-db-instances  --query 'ReservedDBInstances[*].LeaseId' --region $region --output text)
    redShiftRN=$(aws redshift describe-reserved-nodes  --region $region --query 'ReservedNodes[*].ReservedNodeId' --output text)
    esRN=$(aws elasticache describe-reserved-cache-nodes  --region $region --query 'ReservedCacheNodes[*].ReservedCacheNodesOfferingId' --output text)
    if [[ -z "$ec2RI" ]]; then
